...
public final class PaneURLBar: NSView, NSTextFieldDelegate, NSMenuDelegate {
  ...
  /// extension's right-click menu. Routed
  ...
  /// snapshot. Only pinned extensions take a slot here; every other
  /// enabled extension is reached from the sidebar's Extensions list,
  /// where clicking a row opens the same popup. Buttons are rebuilt
  /// rather than diff-applied because the row is bounded by the user's
  /// pinned-extension count and rebuilding is simpler than tracking
  /// stable identity across NSStackView arranged subviews.
  ...
  /// Diameter of the pinned-action buttons. Matches the existing
  /// zoom-indicator buttons so the row reads as a single rhythm of
  /// 22pt squares.
