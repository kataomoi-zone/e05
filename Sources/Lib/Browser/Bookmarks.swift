...
public final class Bookmarks {
    ...
    public func add(url: String, title: String) -> Bool {
        ...
        let sql = "INSERT INTO bookmarks (url, title, created_at) VALUES (?, ?, ?) ON CONFLICT(url) DO UPDATE SET title = excluded.title"
    ...
    public func all() -> [Entry] {
        query(sql: "SELECT id, url, title, created_at FROM bookmarks ORDER BY created_at DESC",
              bind: { _ in })
    }
